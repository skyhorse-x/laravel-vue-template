...
async function handleSubmit() {
  ...
  loading.value = true
  try {
    const res = await userStore.resetPassword(form.value.email, form.value.token, form.value.password)
    if (res.code === 200) {
      ...
      <div class="reset-form-wrapper">
        ...
        <form class="reset-form" @submit.prevent="handleSubmit">
          ...
          <button type="submit" class="btn-primary" :disabled="loading">
            ...
            <span v-else>{{ t('auth.resetBtn') }}</span>
          </button>
        </form>
...
.reset-form-wrapper { width: 100%; max-width: 420px; }
...
@media (max-width: 768px) { .reset-left { display: none; } }
</style>
